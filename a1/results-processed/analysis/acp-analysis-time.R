...
data <- aggregate(list(raw_data$trial_time), by=list(raw_data$participant_id, raw_data$technique, raw_data$granularity, raw_data$open_windows), FUN=mean, na.rm=TRUE)

# Renames column header in the aggregated raw_dataset
colnames(data) = c("Participant", "Technique", "Granularity", "OpenWindows", "Time")
...
ezANOVA(data, dv=Time, wid=Participant, within=c(Technique, Granularity, OpenWindows))

phraseOnly <- subset(data, Granularity=='phrase')
pairwise.t.test(phraseOnly$Time, phraseOnly$Technique, p.adjust="bonf", paired=TRUE)

sentenceOnly <- subset(data, Granularity=='sentence')
pairwise.t.test(sentenceOnly$Time, sentenceOnly$Technique, p.adjust="bonf", paired=TRUE)

paragraphOnly <- subset(data, Granularity=='paragraph')
pairwise.t.test(paragraphOnly$Time, paragraphOnly$Technique, p.adjust="bonf", paired=TRUE)

twoOpenWindows <- subset(data, OpenWindows=='2')
pairwise.t.test(twoOpenWindows$Time, twoOpenWindows$Technique, p.adjust="bonf", paired=TRUE)

fourOpenWindows <- subset(data, OpenWindows=='4')
pairwise.t.test(fourOpenWindows$Time, fourOpenWindows$Technique, p.adjust="bonf", paired=TRUE)

sixOpenWindows <- subset(data, OpenWindows=='6')
pairwise.t.test(sixOpenWindows$Time, sixOpenWindows$Technique, p.adjust="bonf", paired=TRUE)
